      ...
      body: Padding(
        ...
        child: Column(children: [
          ...
          if (chartType.value == 0)
            Expanded(child: _Histogram(statisticType: statisticType, selectedMonth: selectedMonth)),
          if (chartType.value == 1)
            Expanded(child: _Pie(statisticType: statisticType, selectedMonth: selectedMonth)),
          Expanded(
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      IconButton(
          onPressed: () {
            selectedMonth.value =
                selectedMonth.value.copyWith(month: selectedMonth.value.month - 1);
          },
          icon: const Icon(FontAwesomeIcons.chevronLeft, size: 18, color: green100)),
      Text(DateFormat("'Tháng' MM/yyyy").format(selectedMonth.value), style: title2),
      IconButton(
          onPressed: () {
            selectedMonth.value =
                selectedMonth.value.copyWith(month: selectedMonth.value.month + 1);
          },
          icon: const Icon(FontAwesomeIcons.chevronRight, size: 18, color: green100))
    ]);
  }
}
...
class StatisticTypeSwitcher extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      height: 36,
      decoration: BoxDecoration(color: light60, borderRadius: BorderRadius.circular(8)),
      child: Row(children: [
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: Text("Chi tiêu",
                  style: title2.copyWith(color: statisticType.value == 0 ? Colors.white : null)),
            ),
          ),
        ),
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8)),
              child: Text("Thu nhập",
                  style: title2.copyWith(color: statisticType.value == 1 ? Colors.white : null)),
            ),
          ),
        )
      ]),
          ...
          child: Stack(
            children: [
              if (totalMoneyAmount != 0)
                Center(child: Text(NumberFormat("#,##0đ").format(totalMoneyAmount), style: title2))
              else
              ...
              Positioned.fill(
                child: PieChart(PieChartData(
                    ...
                    sections: (statisticType.value == 0
                            ? ref.watch(getExpenseByMonthProvider(selectedMonth.value)).valueOrNull
                            : ref.watch(getIncomeByMonthProvider(selectedMonth.value)).valueOrNull)
                        ?.mapIndexed((i, e) {
                      final isTouched = expandedAll.value || touchedIndex.value == i;
                      ...
                      return PieChartSectionData(
                          radius: radius,
                          showTitle: isTouched,
                          title: "${(moneyAmount / totalMoneyAmount * 100).round()}%",
                          titleStyle: title3,
                          ...
                          badgeWidget: Consumer(builder: (context, ref, child) {
                            ...
                            return cat == null
                                ...
                                : Container(
                                    ...
                                    child:
                                        Icon(getIconDataFromStr(cat.iconType, cat.icon), size: 16),
                                  );
                          }),
...
class _Histogram extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final numberOfColumns =
        getNumberOfDaysInAMonth(year: selectedMonth.value.year, month: selectedMonth.value.month);
    final maxY = statisticType.value == 0
        ? ref.watch(getExpenseChartHeightByMonthProvider(selectedMonth.value)).valueOrNull
        : ref.watch(getIncomeChartHeightByMonthProvider(selectedMonth.value)).valueOrNull;
            ...
            child: maxY == 0
                ...
                : BarChart(BarChartData(
                                ...
                                getTitlesWidget: (value, meta) => Row(
                                      ...
                                      children: [
                                        Text(NumberFormat.compact().format(value),
                                            style: tiny.copyWith(color: Colors.black26)),
                                        Container(
                    ...
                    barGroups: List.generate(
                        numberOfColumns,
                        (index) => BarChartGroupData(barsSpace: 2, x: index + 1, barRods: [
                              if (statisticType.value == 0)
                                BarChartRodData(
                                    toY: ref
                                            .watch(getExpenseByDateProvider(DateTime(
                                                selectedMonth.value.year,
                                                selectedMonth.value.month,
                                                index + 1)))
                                            .valueOrNull
                                            ?.fold(0.0, (val, e) => val + e.moneyAmount)
                                            .toDouble() ??
                                        0,
                                    color: red100,
                              ...
                              if (statisticType.value == 1)
                                BarChartRodData(
                                    toY: ref
                                            .watch(getIncomeByDateProvider(DateTime(
                                                selectedMonth.value.year,
                                                selectedMonth.value.month,
                                                index + 1)))
                                            .valueOrNull
                    ...
                    barTouchData: BarTouchData(
                      ...
                      tooltipPadding: const EdgeInsets.only(top: 4, left: 4, right: 4),
                      getTooltipColor: (group) => green100.withOpacity(0.2),
                      getTooltipItem: (group, groupIndex, rod, rodIndex) {
                        return BarTooltipItem(
                            NumberFormat.compact().format(rod.toY), regular.copyWith(height: 1.2));
                      },
                    )))),
          ),
        ),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return SingleChildScrollView(
      ...
      ...?(statisticType.value == 0
          ...
          ?.map((e) {
        final id = e is ExpenseData ? e.id : (e as IncomeData).id;
        final catId = e is ExpenseData ? e.expenseCategoryId : (e as IncomeData).incomeCategoryId;
        final moneyAmount = e is ExpenseData ? e.moneyAmount : (e as IncomeData).moneyAmount;
        final dynamic cat = ref
            .watch((e is ExpenseData
                ? getExpenseCategoryByIdProvider
                : getIncomeCategoryByIdProvider)(catId))
